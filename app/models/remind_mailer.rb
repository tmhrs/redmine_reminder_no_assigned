class RemindMailer < Mailer
  def self.reminders_no_assigned(options={})
    ...
    issues_by_project.each do |target_project, issues|
      ...
      reminder_no_assigned(target_project.id, mail_address_list, issues, mins).deliver
    end
  end

  def reminder_no_assigned(project_id, mail_address_list, issues, mins)
    set_language_if_valid Setting.default_language
    @issues = issues
    @mins = mins
    @issues_url = url_for(:controller => 'issues', :action => 'index',
                          :set_filter => 1, :project_id => project_id,
                          :assigned_to_id => '!*', :sort => 'created_on')
    mail :to => mail_address_list,
      :subject => l(:mail_subject_reminder_no_assigned, :count => issues.size, :mins => mins)
  end
end
